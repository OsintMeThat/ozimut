...
| **Panorama** | Stitch a video time window (or a set of frames) into one wide view — `cv2.Stitcher` with fallbacks. ✅ **Pulled early into Inspect** (like Frame Extractor): an **auto-stitch** button on the collage that complements the existing hand-made warp collage — machine stitch first, hand-tune after. Still to come: **stitch a video time window** directly (auto-sample the frames), and optional **seam/exposure blending** — the collage path leaves piece edges hard. |
...
- ✅ **Settings & secrets**: an in-app options page, split into tabs —
  **Preferences** (coordinate format DD/DMS/MGRS, metric/imperial units,
  ...
  proof PNG you signed), **Imagery** (optional API
  keys/secrets, stored locally and **never** bundled into a shared case),
  **Usage** (per-provider **monthly counters** so a user tracks a keyed quota
  before hitting it, + eco mode), **About** (version, workspace path, license,
  links, + **Downloaders** ✅ — one-click update for yt-dlp/gallery-dl, see
  below). Core features never require a key (principle 7); key-less providers
